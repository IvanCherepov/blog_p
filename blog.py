...
##
    ...
    def get(self, post_id):
        ...
        #post = db.GqlQuery("SELECT * FROM Post WHERE name = :1",post_id).get()
        #id = post.key().id()
...
##        post = Post.get(key)
        ...
        #post = Post.get_by_key_name(str(post_id))
        ...
        if not post:
            self.redirect('/_edit/%s' % str(post_id))
            return
        if self.format == 'html':
...
class EditPage(BlogHandler):
    def get(self,post_id):
        ...
        if self.user:
            if post:
                self.render("newpost.html", content = post.content)
            elif not post:
                self.render("newpost.html")
        elif not self.user:    
            self.redirect("/")
...
##            p.put()
            ...
            p = Post(name = post_id, content = content)
            p.put()
